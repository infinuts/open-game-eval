## LLM Leaderboard on Roblox Studio Assistant — 47-Eval Set (DEPRECATED)

> **⚠ DEPRECATED.** This leaderboard is based on the original 47-eval set. It is retained for historical reference only. The **active leaderboard** is the [87-eval expanded set in `LLM_LEADERBOARD.md`](LLM_LEADERBOARD.md), which adds 40 new evals covering more complex game mechanics (scripting, physics, multi-instance edits, client-server interactions) and is the canonical benchmark going forward. New model entries should go in `LLM_LEADERBOARD.md`, not here.
