...
local eval: BaseEval = {
	scenario_name = "017_make_traffic_light_bug_2",
	prompt = { "I added the traffic light model to my game, but it's just stuck. The lights never turn on or change color." },
...
-- Bug: Create the correct script, but leave it disabled.
local trafficLight = workspace:WaitForChild("TrafficLight")
local lightScript = Instance.new("Script", trafficLight)
lightScript.Name = "TrafficController"
lightScript.Disabled = true -- The bug is that the script is disabled.
lightScript.Source = [[
