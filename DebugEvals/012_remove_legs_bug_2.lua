...
local eval: BaseEval = {
	scenario_name = "012_remove_legs_bug_2",
	prompt = { "When I join the game, my character's legs are supposed to be removed, but they're still there. The script doesn't seem to be working." },
    ...
    local function removeLegs(character)
        if character then
            for _, v in ipairs(character:GetChildren()) do
                if v:IsA("BasePart") and (string.lower(v.Name):find("leg") and string.lower(v.Name):find("foot")) then
                    v:Destroy()
                end
            end
        end
...
-- Clean up the old buggy script first
local ServerScriptService = game:GetService("ServerScriptService")
local oldScript = ServerScriptService:FindFirstChild("RemoveLegs")
if oldScript then
    oldScript:Destroy()
end
