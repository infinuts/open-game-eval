...
-- Bug: A previous script incorrectly set the smoke color to black.
...
eval.check_scene = function()
	...
	for _, smoke in ipairs(game:GetService("Workspace"):GetDescendants()) do
		if
			(smoke:IsA("Smoke") or smoke:IsA("ParticleEmitter"))
			and (smoke.Parent:IsA("BasePart") or smoke.Parent:IsA("Attachment"))
		then -- smoke lives in either parts or attachemnets
			for i, chimney in ipairs(GlobalTable.chimneyLocations) do
				...
				if typeof(smoke.Color) == "ColorSequence" then
					for _, keypoint in pairs(smoke.Color.Keypoints) do
						if
							keypoint.Value.R >= (240 / 255)
							and keypoint.Value.G >= (240 / 255)
							and keypoint.Value.B >= (240 / 255)
						then
							isWhite = true
							break
						end
					...
					isWhite = smoke.Color.R >= (240 / 255)
						and smoke.Color.G >= (240 / 255)
						and smoke.Color.B >= (240 / 255) -- all should be above 240 for white, leaves good wiggle room
				end
	...
	for _, v in ipairs(GlobalTable.chimneyLocations) do
		print(v)
	end
